...
function ResolveModal({ alert, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "#1a1a26", border: "1px solid #2a2a3a", borderTop: `3px solid ${c}`, borderRadius: 16, padding: 28, width: "100%", maxWidth: 480 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: c + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{icons[alert.type] || "⚠️"}</div>
          <div>
        ...
        <div style={{ marginBottom: 20 }}><LiveTimer triggeredAt={alert.triggeredAt} /></div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
          {suggestions.map((s) => (
            <button key={s} onClick={() => setNote(s)} style={{ padding: "5px 10px", borderRadius: 20, fontSize: 11, cursor: "pointer", background: note === s ? c + "25" : "#12121c", color: note === s ? c : "#666", border: `1px solid ${note === s ? c + "60" : "#2a2a3a"}` }}>{s}</button>
          ))}
        </div>
        <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Describe how the situation was resolved..." rows={3}
          style={{ width: "100%", background: "#12121c", border: "1px solid #2a2a3a", borderRadius: 10, padding: "10px 12px", color: "#e8e8f0", fontSize: 13, resize: "none", outline: "none", boxSizing: "border-box", fontFamily: "'Space Grotesk', sans-serif" }} />
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button onClick={onCancel} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #2a2a3a", background: "transparent", color: "#666", fontSize: 13, cursor: "pointer" }}>Cancel</button>
          <button onClick={async () => { if (!note.trim()) return; setLoading(true); await onConfirm(note); setLoading(false); }} disabled={!note.trim() || loading}
            style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: note.trim() ? "#30d158" : "#1e3a28", color: note.trim() ? "white" : "#444", fontSize: 13, fontWeight: 700, cursor: note.trim() ? "pointer" : "not-allowed" }}>
...
export default function App() {
  ...
  useEffect(() => { loginAnonymously().then(setUserId).catch(() => {}); }, []);
  ...
  const handleSOS = useCallback(async (payload) => {
    try {
      ...
      return result;
    } catch (err) { show("SOS failed: " + err.message, "critical"); throw err; }
  }, [userId, show]);
  ...
  const handleResolve = useCallback(async (docId, note) => {
    ...
    } catch (err) { show("Failed to resolve: " + err.message, "critical"); }
  }, [userId, show]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e8f0", fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      {resolvingAlert && <ResolveModal alert={resolvingAlert} onConfirm={(note) => handleResolve(resolvingAlert.docId, note)} onCancel={() => setResolvingAlert(null)} />}
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(18,18,26,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid #2a2a3a" }}>
        <div className="header-inner" style={{ padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          ...
          <div className="header-roles" style={{ display: "flex", gap: 4 }}>
            {["admin", "staff", "guest", "responder"].map((r) => (
              <button key={r} onClick={() => setRole(r)} style={{ padding: "6px 14px", borderRadius: 6, fontSize: 12, fontWeight: 500, border: "1px solid transparent", cursor: "pointer", textTransform: "capitalize", background: role === r ? "rgba(255,45,85,0.1)" : "transparent", color: role === r ? "#ff2d55" : "#666", borderColor: role === r ? "rgba(255,45,85,0.2)" : "transparent" }}>{r}</button>
            ))}
          </div>
        ...
        <div className="mobile-roles" style={{ display: "none", borderTop: "1px solid #1a1a2a" }}>
          {["admin", "staff", "guest", "responder"].map((r) => (
            <button key={r} onClick={() => setRole(r)} style={{ flex: 1, padding: "10px 4px", fontSize: 11, fontWeight: 600, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: 0.5, background: role === r ? "rgba(255,45,85,0.1)" : "transparent", color: role === r ? "#ff2d55" : "#555", borderBottom: role === r ? "2px solid #ff2d55" : "2px solid transparent" }}>{r}</button>
          ))}
        </div>
      </header>
                  ...
                  return (
                    <div key={a.docId} style={{ background: "#1a1a26", border: "1px solid #2a2a3a", borderTop: "3px solid " + c, borderRadius: 12, padding: 20 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#888", display: "flex", flexDirection: "column", gap: 4, marginBottom: 16 }}>
                        ...
                        <span>🕐 Triggered at {a.triggeredAt?.toLocaleTimeString()}</span>
                      </div>
                      <button onClick={() => setResolvingAlert(a)} style={{ width: "100%", padding: 10, borderRadius: 10, border: "none", background: "#30d158", color: "white", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>✓ Mark Resolved</button>
                    </div>
                  );
